...
import { Card, CardHeader, CardActionArea, CardContent, CircularProgress } from '@mui/material';
    ...
    return <div className={styles.commitList}>{
        sortedCommitDataList.map(commitData => (
            <Card sx={{ margin: '10px' }} key={commitData.sha}>
                <CardActionArea onClick={() => window.open(`https://github.com/mercari/${commitData.repoName}/commit/${commitData.sha}`)}>
